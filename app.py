...
# Add option to use CPU only or lower memory footprint
use_cpu = os.environ.get('USE_CPU', '0') == '1'
device = 'cpu' if use_cpu else 'cuda'
low_memory = os.environ.get('LOW_MEMORY', '0') == '1'

print(f"Using device: {device}, Low memory mode: {low_memory}")

# Initialize models with selected device and memory optimization
cfm, tokenizer, muq, vae = prepare_model(device, low_memory=low_memory)
if not use_cpu:
    # Only compile when using CUDA
    try:
...
@spaces.GPU
def infer_music(lrc, ref_audio_path, steps, file_type, max_frames=2048, device=device):
    try:
        ...
        # Convert tensor to numpy for compatibility with Gradio
        audio_np = generated_song.numpy()
        
        # Gradio expects (sample_rate, audio_data) or a file path
        sample_rate = 44100  # Standard sample rate
        
        # Save to a temporary file if needed
        import tempfile
        output_path = tempfile.mktemp(suffix=f".{file_type}")
